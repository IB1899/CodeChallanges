//! The password
let password = document.querySelector(".password") as HTMLHeadElement;

//! Copy password button
let CopyButton = document.querySelector("#copy") as HTMLSpanElement;
...
let passwordSlider = document.querySelector("#slider") as HTMLInputElement;

//! Span to show the value of the "passwordSlider".
let PasswordLengthShow = document.querySelector("#PasswordLength") as HTMLSpanElement;

//! A bar to show the password strength.
let PasswordStrength = document.querySelector(".PasswordStrength") as HTMLDivElement;
...
let ExcludeDuplicates = document.querySelector("#ExcludeDuplicates") as HTMLInputElement;

let options = document.querySelectorAll<HTMLInputElement>(".PasswordSettings input");


//! The generate password button.
let GeneratePasswordButton = document.querySelector(".generate") as HTMLButtonElement;

//* To invoke its properties based on the option.id
let charactersConditions: any = {
...
let GeneratePassword = () => {
    ...
    if (characters.length <= 38) { ExcludeDuplicates.checked = false }

    //* Loop according to the length of the passwordSlider
    for (let i = 0; i < passwordSlider.valueAsNumber; i++) {

        let randomCharacter: string = characters.charAt(Math.floor(Math.random() * characters.length))
